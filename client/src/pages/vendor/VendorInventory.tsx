...
export default function VendorInventory() {
  ...
  return <DashboardLayout items={vendorNavigation} workspaceLabel="Vendor workspace"><WorkspaceGate allowedRoles={["vendor_staff", "platform_admin", "admin"]}><div className="mx-auto max-w-7xl"><PageIntro eyebrow="VARIANT INVENTORY" title="Stock by size, ready for pickup." description="Every size is tracked independently. Change only the current quantity and the number of units that should trigger a low-stock alert." /><div className="mt-7 grid gap-3 sm:grid-cols-2"><section className="campus-panel flex gap-3 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><Boxes className="size-5" aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Variant records</p><p className="mt-1 text-xl font-extrabold tabular-nums">{inventory.data?.length ?? 0}</p></div></section><section className="campus-panel flex gap-3 border-amber-200 bg-amber-50/45 p-4"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-amber-700 shadow-sm"><CircleAlert className="size-5" aria-hidden="true" /></span><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground">Need attention</p><p className="mt-1 text-xl font-extrabold tabular-nums text-amber-800">{lowStockCount}</p></div></section></div>{inventory.isLoading ? <Skeleton className="mt-5 h-72 rounded-2xl" /> : inventory.isError ? <div className="mt-5"><EmptyPanel title="Inventory could not be loaded" detail="Please try again to view variant stock." action={{ label: "Try again", onClick: () => inventory.refetch() }} /></div> : inventory.data?.length ? <><section className="mt-5 md:hidden" aria-label="Mobile inventory records"><div className="mb-3 rounded-xl border border-primary/10 bg-secondary/40 px-4 py-3 text-xs leading-5 text-muted-foreground">Use <strong className="text-foreground">Available now</strong> for the physical quantity and <strong className="text-foreground">Low-stock alert</strong> for the threshold that needs attention.</div><div className="space-y-3">{inventory.data.map(item => <InventoryCard key={item.variantId} item={item} inventoryQueryKey={inventoryKey} />)}</div></section><section className="campus-panel mt-5 hidden overflow-x-auto md:block" aria-labelledby="inventory-table-title"><div className="flex items-end justify-between gap-4 border-b border-border px-5 py-4"><div><p className="campus-eyebrow">STOCK LEDGER</p><h2 id="inventory-table-title" className="mt-1 font-extrabold">Size-level inventory</h2></div><p className="text-xs leading-5 text-muted-foreground">Edit the physical stock and alert threshold, then save the individual size row.</p></div><table className="min-w-[800px] w-full text-left text-sm"><thead className="border-b border-border bg-muted/60 text-xs font-bold uppercase tracking-[0.08em] text-muted-foreground"><tr><th className="px-5 py-3">Product</th><th className="px-4 py-3">Size</th><th className="px-4 py-3">Availability</th><th className="px-4 py-3">Stock</th><th className="px-4 py-3">Alert at</th><th className="px-5 py-3 text-right">Action</th></tr></thead><tbody>{inventory.data.map(item => <InventoryRow key={item.variantId} item={item} inventoryQueryKey={inventoryKey} />)}</tbody></table></section></> : <div className="mt-5"><EmptyPanel title="No variants yet" detail="Create a product with sizes to manage its inventory here." /></div>}</div></WorkspaceGate></DashboardLayout>;
}
